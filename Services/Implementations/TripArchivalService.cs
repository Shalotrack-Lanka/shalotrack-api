    ...
    private async Task<DateTime> ResolveTripStartAsync(Guid deviceId, DateTime tripEndTime)
    {
        ...
        if (ignitionOnAlert is not null)
        {
            ...
            if (candidateAge <= FallbackMaxTripDuration)
            {
                ...
                // bogus multi-day "trip". NOTE: this used to intermittently
                // misfire on the trip's OWN closing IgnitionOff due to a
                // timestamp race in LocationNotificationListener -- fixed
                // 2026-09-19 (see that class). ExistsByDeviceAndTypeBetweenAsync
                // is strictly exclusive on both bounds, and tripEndTime is now
                // always exactly equal to the closing alert's own TriggeredAt,
                // so it can never match itself here.
